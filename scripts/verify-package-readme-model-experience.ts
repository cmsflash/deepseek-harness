...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/speech/speech': { kind: 'none', reason: 'Speech is presentation: the seam registers no prompt or schema, and no synthesized audio reaches a model request.' },
  'packages/speech/speech-openai-compatible': { kind: 'none', reason: 'The provider backend is reached only through ctx.speech, which registers nothing model-facing.' },
  'packages/speech/speech-cache': { kind: 'none', reason: 'Cached read-aloud audio never enters the session log or a model request; the model cannot observe whether a reply was spoken.' },
  'packages/client/ui-message-speech': { kind: 'none', reason: 'Browser-side playback control; registers nothing model-facing.' },
