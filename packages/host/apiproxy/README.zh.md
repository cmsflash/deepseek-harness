...
`stepDetail: 'collapsed'` 把可省略的 step 只提供为它的 `step/start`／`step/end` 边界外加一个 `StepDigest`——该 step 的完整模型调用数、工具调用数、token、墙钟时间与 diff 行数总计——并扣下其内部内容。只有折叠阅读者不在看的 step 才可省略：一个 turn 的最高 step 以及承载其收尾 assistant 文本的 step 始终完整提供，不带 step 坐标的事件同样如此。是否保留按整个事件区间而非按页决定，因此跨页拆分的 turn 在两页上保持完整的是同一批 step。边界永不省略，因此每一页仍是一段连续区间。`session.expandSteps` 回读单个 turn 被扣下的事件，其下界为调用方的 `fromSeq`，因此比调用方窗口更早的 turn 无法把事件拼接到该窗口头部之下（[决策](../../../.agents/notes/implemented/architecture/2026-09-01-collapsed-step-digest-paging.zh.md)）。
