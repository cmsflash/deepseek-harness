...
export interface IApiClient {
  sessions: {
    ...
    expandSteps(payload: RequestPayload<'session.expandSteps'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.expandSteps'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.expandSteps': sessionExpandStepsValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    expandSteps: (payload, signal) => this.callUnary('session.expandSteps', payload, signal),
