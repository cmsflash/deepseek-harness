...
import { collapseSteps, diffLineDelta, elidedEventsOfTurn } from '@deepseek-ai/dsh-host-apiproxy/api/step-collapse'

let seq = 0

function event<T extends string>(type: T, data: unknown, time = 1000): SessionEvent {
...
function rows(events: readonly SessionEvent[]): { event: SessionEvent }[] {
  return events.map(item => ({ event: item }))
}

function types(result: readonly { event: SessionEvent }[]): string[] {
  return result.map(row => row.event.type)
}
...
describe('collapseSteps', () => {
  it('keeps the turn\'s last step whole and reduces earlier ones to their boundaries', () => {
    ...
    const { rows: served, digests } = collapseSteps(rows(log), log)
  ...
  it('never elides an event that carries no step coordinate', () => {
    ...
    const { rows: served } = collapseSteps(rows(log), log)
  ...
  it('retains the step carrying the turn\'s closing text even when a later step exists', () => {
    ...
    const texts = served
      .filter(row => row.event.type === 'assistant/message')
      .map(row => (row.event.data as { message: { content: { text?: string }[] } }).message.content[0]?.text)
  ...
  it('decides retention per turn, so each turn keeps its own last step', () => {
    ...
    const { digests } = collapseSteps(rows(log), log)
  ...
  it('counts diff line volume from the render intent the same page computed', () => {
    seq = 0
    const log = turn(1, [
      [
        event('step/start', { turn: 1, step: 1 }),
        event('tool/call', { turn: 1, step: 1, callId: 'c1', name: 'write', arguments: '{}' }),
        event('tool/result', { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, content: [] } }),
        event('step/end', { turn: 1, step: 1 }),
      ],
      step(1, 2, { text: 'done' }),
    ])
    const page = log.map((item) => {
      if (item.type !== 'tool/result') return { event: item }
      return {
        event: item,
        view: { for: 'result', view: { card: 'diff', diffs: [{ path: 'a.ts', oldText: 'x\n', newText: 'x\ny\n' }] } },
      }
    })
    const { digests } = collapseSteps(page, log)
  ...
  it('decides retention over the whole log, not the page, so a split turn agrees across pages', () => {
    ...
    const { digests } = collapseSteps(rows(cut), log)
  ...
  it('addresses a step whose start fell on an earlier page by its first elided event', () => {
    ...
    const { digests } = collapseSteps(rows(interior), log)
...
describe('elidedEventsOfTurn', () => {
  it('returns exactly what a collapsed page withheld for that turn', () => {
    ...
    const servedSeqs = new Set(served.map(row => row.event.seq))
  ...
  it('returns exactly the page\'s withheld rows when bounded to that page', () => {
    ...
    const { rows: served } = collapseSteps(rows(page), log)
