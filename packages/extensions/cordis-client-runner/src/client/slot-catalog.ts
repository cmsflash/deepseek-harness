...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.attach',
    kind: 'single',
    scope: 'session',
    summary: 'The named attach seat in the composer tool row, immediately right of the plan control — one occupant, so taking it means rendering the whole attach affordance yourself.',
    doc: 'The named attach seat in the composer tool row, immediately right of the\nplan control — one occupant, so taking it means rendering the whole\nattach affordance yourself. The owner passes the composer\'s own image\nintake (see ComposerAttachOwnerProps), so a click-to-browse\nentry reuses the validation and rejection notices that already back\npaste and drop. Unoccupied, the seat renders nothing, leaving paste and\ndrop as the only intake gestures.',
    registerOptions: [],
    ownerProps: [
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-conversation CollapseStepsRow id \'chat-collapse-steps\'',
