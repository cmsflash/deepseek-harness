...
import TtsRuntime, {
  ...
  type SpeechSpec,
} from '@deepseek-ai/dsh-tts'

const BASE: TtsRuntimeConfig = {
...
async function mount(config: Partial<TtsRuntimeConfig> = {}): Promise<TtsRuntime> {
  const ctx = new Context()
  await ctx.plugin(TtsRuntime, { ...BASE, ...config })
  return ctx.tts
}

describe('TtsRuntime.resolve', () => {
...
describe('TtsRuntime provider selection', () => {
  ...
  it('removes a contribution when the registering fiber unloads', async () => {
    const ctx = new Context()
    await ctx.plugin(TtsRuntime, BASE)
    const fiber = await ctx.plugin({
      inject: ['tts'],
      apply: (inner: Context) => { inner.tts.registerProvider(provider('scoped', true)) },
    })
    await expect(ctx.tts.synthesize({ text: 'hi' })).resolves.toEqual(audio('scoped'))
    await fiber.dispose()
    await expect(ctx.tts.synthesize({ text: 'hi' })).rejects.toMatchObject({
      code: 'SPEECH_PROVIDER_UNAVAILABLE',
    })
  })
