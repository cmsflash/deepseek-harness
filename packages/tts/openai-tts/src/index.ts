/**
 * `@deepseek-ai/dsh-openai-tts`: registers one
 * {@link OpenAiTtsProvider} per configured route with `ctx.tts`.
 * A function/namespace plugin (NOT a default-export service): a provider does
 * not own the `ctx.tts` key — it registers INTO the seam's registry, exactly
 * as `@deepseek-ai/dsh-web-search-exa` registers into `ctx.web`. The key is
 * owned by `@deepseek-ai/dsh-tts`.
 *
 ...
 * ```yaml
 * - id: speech-routes
 *   name: '@deepseek-ai/dsh-openai-tts'
 *   config:
 *     providers:
 ...
 * ```
 * @module @deepseek-ai/dsh-openai-tts
 */
...
export { OpenAiTtsProvider } from './provider.ts'
export type { OpenAiTtsProviderOptions } from './provider.ts'

/** Cordis plugin name used by loader diagnostics. */
export const name = 'openai-tts'
...
/**
 * Register one speech provider per configured route with `ctx.tts`.
 *
...
export function apply(ctx: Context, config: Config): void {
  ...
  for (const [id, route] of Object.entries(config.providers)) {
    ...
    ctx.tts.registerProvider(new OpenAiTtsProvider({
      id,
