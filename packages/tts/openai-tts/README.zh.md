...
为每一条配置的路由向 `ctx.tts` 注册一个 `SpeechProvider`，各自以其路由的 map 键为 id。它不拥有 `ctx.tts` 键——它注册进 seam 的注册表，正如 `@deepseek-ai/dsh-web-search-exa` 注册进 `ctx.web`。该键由 [`@deepseek-ai/dsh-tts`](../tts/README.zh.md) 拥有。
...
无，因为本提供方只经 `ctx.tts` 触达，而后者不贡献任何提示词或 schema，也没有合成音频到达模型请求。
