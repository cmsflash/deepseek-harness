/**
 * ReadAloudService over a real cordis Context with a scripted speech seam:
...
import ReadAloudService from '@deepseek-ai/dsh-read-aloud'
...
async function bench(options: { synthesizeOnTurnEnd?: boolean; fail?: boolean } = {}) {
  ...
  class TtsService extends Service {
    constructor(serviceCtx: Context) { super(serviceCtx, 'tts') }
  ...
  const fiber = ctx.plugin(ReadAloudService, {
    ttlDays: 7,
...
describe('ReadAloudService turn/end trigger', () => {
  ...
  it('falls back to the home directory when DSH_HOME is unset', async () => {
    ...
    new SessionsService(ctx)
    const fiber = ctx.plugin(ReadAloudService, { ttlDays: 7, synthesizeOnTurnEnd: false })
    await fiber.await()
    expect(ctx.readAloud).toBeDefined()
    await fiber.dispose()
  })
...
describe('ReadAloudService.audio', () => {
  it('serves cached audio without re-synthesizing', async () => {
    ...
    await vi.waitFor(async () => {
      expect(await readdir(join(b.home, 'cache', 'read-aloud'))).toHaveLength(1)
    })

    const reply = await b.ctx.readAloud.audio({ sessionId: SessionId('s1'), messageId: MessageId('m1') })
  ...
  it('reports an unknown session distinctly from an unknown message', async () => {
    const b = await bench({ synthesizeOnTurnEnd: false })
    await expect(b.ctx.readAloud.audio({ sessionId: SessionId('nope'), messageId: MessageId('m1') }))
      .resolves.toMatchObject({ ok: false, code: 'session-not-found' })
    b.sessions.set('s1', session('s1', [assistant(1, 1, 'm1', [{ type: 'text', text: 'x' }])]))
    await expect(b.ctx.readAloud.audio({ sessionId: SessionId('s1'), messageId: MessageId('absent') }))
  ...
  it('reports provider failure without throwing at the Remote', async () => {
    ...
    b.sessions.set('s1', session('s1', [assistant(1, 1, 'm1', [{ type: 'text', text: 'x' }])]))
    await expect(b.ctx.readAloud.audio({ sessionId: SessionId('s1'), messageId: MessageId('m1') }))
