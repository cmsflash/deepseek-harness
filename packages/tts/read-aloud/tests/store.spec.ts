...
import { ReadAloudStore } from '@deepseek-ai/dsh-read-aloud'

const DAY_MS = 86_400_000
const created: string[] = []

async function makeStore(ttlMs = 7 * DAY_MS): Promise<{ store: ReadAloudStore; dir: string }> {
  const dir = await mkdtemp(join(tmpdir(), 'dsh-read-aloud-'))
  created.push(dir)
  return { store: new ReadAloudStore(join(dir, 'read-aloud'), ttlMs), dir }
}
...
describe('ReadAloudStore', () => {
  ...
  it('treats an entry past the retention window as a miss', async () => {
    ...
    await backdate(join(dir, 'read-aloud', 'old.mp3'), 2 * DAY_MS)
  ...
  it('leaves no staging files behind', async () => {
    ...
    await store.write(MessageId('m1'), new Uint8Array([1]))
    const entries = await readdir(join(dir, 'read-aloud'))
  ...
  it('sweeps only expired artifacts', async () => {
    ...
    await backdate(join(dir, 'read-aloud', 'stale.mp3'), 3 * DAY_MS)
    await expect(store.sweep()).resolves.toBe(1)
    expect(await readdir(join(dir, 'read-aloud'))).toEqual(['fresh.mp3'])
  })

  it('ignores unrelated files while sweeping', async () => {
    const { store, dir } = await makeStore(DAY_MS)
    await store.write(MessageId('stale'), new Uint8Array([1]))
    const other = join(dir, 'read-aloud', 'notes.txt')
    await writeFile(other, 'keep me')
    await backdate(join(dir, 'read-aloud', 'stale.mp3'), 3 * DAY_MS)
    await backdate(other, 3 * DAY_MS)
    await expect(store.sweep()).resolves.toBe(1)
    expect(await readdir(join(dir, 'read-aloud'))).toEqual(['notes.txt'])
  })
...
describe('ReadAloudStore failure paths', () => {
  it('treats an unreadable artifact as a miss', async () => {
    ...
    await rm(join(dir, 'read-aloud', 'm1.mp3'))
    await mkdir(join(dir, 'read-aloud', 'm1.mp3'))
  ...
  it('removes the staging file when publication fails', async () => {
    ...
    await mkdir(join(dir, 'read-aloud'), { recursive: true })
    await mkdir(join(dir, 'read-aloud', 'm1.mp3'), { recursive: true })
    await mkdir(join(dir, 'read-aloud', 'm1.mp3', 'child'), { recursive: true })
    await expect(store.write(MessageId('m1'), new Uint8Array([1]))).rejects.toThrow()
    const remaining = await readdir(join(dir, 'read-aloud'))
  ...
  it('skips a listed entry whose target no longer exists', async () => {
    ...
    await backdate(join(dir, 'read-aloud', 'gone.mp3'), 3 * DAY_MS)
    // A dangling symlink is listed but cannot be stat'ed, exactly like an
    // artifact removed between the listing and the stat.
    await symlink(join(dir, 'read-aloud', 'missing-target'), join(dir, 'read-aloud', 'vanished.mp3'))
    await expect(store.sweep()).resolves.toBe(1)
  })
  ...
  it('counts only the artifacts it actually removed', async () => {
    ...
    await mkdir(join(dir, 'read-aloud', 'blocked.mp3'), { recursive: true })
    await mkdir(join(dir, 'read-aloud', 'blocked.mp3', 'child'), { recursive: true })
    await backdate(join(dir, 'read-aloud', 'blocked.mp3'), 3 * DAY_MS)
    await expect(store.sweep()).resolves.toBe(1)
  })
})
