...
The bounded historical normalizers convert `steering/message` to `user/message`, rename `compact/*` events to `compaction/*`, remove `turn/start.trigger`, convert retired `turn/end` reasons, add current message wrappers and deterministic ids for legacy messages, retry chains, and compaction groups, remove the obsolete `request/header.header.messagePrefix` duplicate, and stamp a released `subagent/descriptor` version 2 payload as version 3 (version 3 only added the optional `agentReasoningEffort` member, so the released key set is unchanged; a version 2 payload carrying that member refuses). Token `usage` on `assistant/message`, `assistant/chunk`, and `compaction/summary` admits the released `costUsd` billed amount as a non-negative finite number beside the integer counts. Retired `request/header-delta`, `mode/set`, the `request/header` fallback reason, and descriptor versions other than 2 and 3 refuse migration. No other event, reference, source, or payload fact may change.
