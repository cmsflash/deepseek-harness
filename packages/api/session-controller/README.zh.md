...
`expandSteps` 读回一个轮次在客户端窗口头部及之后省略的事件。向 `page` 提供 `fromSeq` 则按完整详情精确读取截至 `throughSeq` 的闭区间，且不激活 Agent。这一区间模式拒绝 `beforeSeq`、消息预算或折叠详情，而不会悄悄截短请求的数据。
