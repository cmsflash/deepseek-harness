...
Both endpoints take `stepDetail`. Under `collapsed`, a step a reader is not looking at — neither its turn's highest step nor the step carrying the turn's closing assistant text, decided over the whole log through the requested cut — keeps only its `step/start` and `step/end` records and gains one `StepDigest` (event count, model and tool calls, files and diff lines from `tool/result.data.meta.diffs`, wall time, and billed tokens); the record after each withheld run carries `covers`, the inclusive seq range it stands for, so a page's records still partition its range. `expandSteps` reads one turn's withheld events back at or after a window head. A page that collapsing would leave empty is served whole ([decision](../../../.agents/notes/implemented/architecture/2026-09-01-collapsed-step-digest-paging.md)).
