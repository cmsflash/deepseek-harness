...
Both endpoints take `stepDetail`. Under `collapsed`, a step a reader is not looking at — neither its turn's highest step nor the step carrying the turn's closing assistant text, decided over the whole log through the requested cut — keeps only its `step/start` and `step/end` records and gains one `StepDigest`; the record after each withheld run carries `covers`, the inclusive seq range it stands for, so a page's records still partition its range. Nested PTC records inherit their root call's step. `expandSteps` reads one turn's withheld events back at or after a window head. A page that collapsing would leave empty is served whole ([decision](../../../.agents/notes/implemented/architecture/2026-09-01-collapsed-step-digest-paging.md)).

A digest accounts for the whole step through the requested cut, not the page: model calls, settled root and nested tool calls, distinct recorded `filePaths`, added and removed lines, wall time, and billed tokens are the same from whichever page carries the step, and only `elided` counts the events that page withheld. Once the step or its turn is closed, a root or nested call start without a settlement counts as an interrupted call; a compaction replacement of an already recorded result adds no call. File figures use the shared [`appliedFileDiffs`](../../core/tools/README.md) reading of the recorded call head, outcome, and `tool/result.data.meta`: a successful `write` whose result persisted no hunk falls back to its whole-file argument image, so a create and an identical overwrite report the same volume. A Client holding several page fragments of one step keeps one account and sums only their `elided` counts.
