...
Each served `turn/end` record also carries `turnUsage`, outside its unchanged durable event. The [token-meter fold](../../llm/token-meter/README.md#turn-accounting) computes the complete turn's tokens, recorded cost, and unpriced-call count through that end, including retry attempts. The result is independent of step detail and whether the turn start is outside the page; invalid or incomplete accounting is `null`, not a partial total. Page and exact-interval reads use only the requested source prefix and attach summaries only to served end records. Follow seeds the same fold from its opening observation, then advances it once per accepted live event. These summaries require neither hidden-step fetches nor a session-wide map on the wire.

Both endpoints take `stepDetail`. Under `collapsed`, a step that is neither its turn's highest step nor the step carrying its closing assistant text keeps its `step/start` and `step/end` records plus a `StepDigest`. Retention is decided over the whole log through the requested cut. Nested PTC records inherit their root call's step. Each retained record's `covers` states the inclusive seq range it represents, keeping the page's coverage contiguous; a page that collapsing would leave empty is served whole ([decision](../../../.agents/notes/implemented/architecture/2026-09-01-collapsed-step-digest-paging.md)).

A digest accounts for the whole step: model calls, root and nested settled tool calls, distinct recorded `filePaths`, line volume, wall time, and billed tokens. Its `elided` count alone describes the events withheld from that page. File figures use the shared `appliedFileDiffs` reading of the recorded call head, outcome, and metadata; a successful write without usable hunks retains the whole-file argument fallback. Repeated page fragments keep one step account and combine only their omitted-event counts.

`expandSteps` reads one turn's withheld events at or after the client's window head. `page` with `fromSeq` instead reads the exact inclusive interval through `throughSeq` at full detail, without activating an Agent. This interval mode rejects `beforeSeq`, a message budget, or collapsed detail rather than silently truncating the requested data.
