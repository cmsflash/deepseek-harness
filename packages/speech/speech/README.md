...
The **`SpeechRuntime`** (`ctx.speech`) defines WHAT speech synthesis the harness has — turn text into audio — over multiple providers, without binding callers to one vendor's request format.
