 ...
 * @module @deepseek-ai/dsh-speech-cache
 */
...
import { SpeechCacheStore } from './store.ts'
 ...
 * carry the text.
 */
export class SpeechCacheService extends TypertRemoteService {
  static inject = ['sessions', 'speech']
  ...
  private readonly store: SpeechCacheStore
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx, 'speechCache')
    const home = process.env.DSH_HOME ?? join(homedir(), '.dsh')
    this.store = new SpeechCacheStore(join(home, 'cache', 'speech'), config.ttlDays * DAY_MS)
  ...
  private async synthesizeAndStore(messageId: MessageId, text: string): Promise<Uint8Array> {
    const audio = await this.ctx.speech.synthesize({ text })
