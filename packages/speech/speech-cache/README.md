...
Read-aloud audio for completed turns: a `turn/end` listener that synthesizes each turn's closing prose through [`ctx.speech`](../speech/README.md), a filesystem cache under the Harness home, and the `speechCache` Remote a browser plays it through.
