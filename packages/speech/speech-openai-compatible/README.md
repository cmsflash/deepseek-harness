...
Registers one `SpeechProvider` per configured route with `ctx.speech`, each under its route's map key. It does not own the `ctx.speech` key — it registers INTO the seam's registry, exactly as `@deepseek-ai/dsh-web-search-exa` registers into `ctx.web`. The key is owned by [`@deepseek-ai/dsh-speech`](../speech/README.md).
