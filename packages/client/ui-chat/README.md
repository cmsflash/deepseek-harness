...
A turn is one user round and each step inside it is one model call, so the reading cost of a long turn is its intermediate steps rather than its answer. In `Collapsed steps` mode every turn keeps only its highest step rendered — the live one while it streams — and folds its earlier `assistant-step` and `tool-call` rows into one `CollapsedStepsRow` reporting the elapsed time, steps, tool calls, tokens, added and removed lines, and files they produced. Rows of any other kind — the prompting message, context injections, the turn tail — never collapse, wherever the log placed them. Opening the row restores the hidden rows in place through the same keyed node seat, so an expanded group renders exactly like an uncollapsed transcript; disclosure is per turn, all-or-nothing, reader-owned, and not persisted. The row is open at one point: `conversation.chat.collapsedMetric` is a session-scoped list slot whose entries render after every figure the row computes itself, so a contributor never reserves an `order` band against figures the row may gain later; the owner passes the collapsed group's turn, the node keys it hides, and its folded step and call counts, so a contributor folds exactly the set the built-in figures fold. Reading the whole turn instead would count the still-visible last step, which no other figure on the line includes. The disclosure control and the figures are siblings, so a contributed figure may carry interactive content without nesting inside a button.

The mode also selects how history is fetched. While it is active, the plugin sets the sessions domain's step detail to `collapsed`: pages request boundaries plus one `StepDigest` per elidable step and withhold the interior until `expandTurn` reads that turn back through `session.expandSteps`, so one page spans far more turns for the same message budget. The row's figures come from those digests, which describe whole steps, so they neither depend on where a page boundary fell nor change when the reader expands; a turn whose steps are all loaded expands without any request, and the row announces an in-flight read-back ([decision](../../../.agents/notes/implemented/architecture/2026-09-01-collapsed-step-digest-paging.md)).
