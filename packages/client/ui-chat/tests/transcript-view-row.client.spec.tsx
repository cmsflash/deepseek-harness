...
describe('TranscriptViewRow', () => {
  it('explains the preference and shows Compact by default', () => {
    ...
    expect(screen.getByText('Controls how turn process content is shown')).toBeDefined()
  ...
  it('offers the collapsed-steps mode and follows it', () => {
    const b = mount()
    fireEvent.click(screen.getByRole('button', { name: /Compact/ }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Collapsed steps' }))
    expect(b.setTranscriptView).toHaveBeenCalledWith('collapsed')
    expect(screen.getByRole('button', { name: /Collapsed steps/ })).toBeDefined()
  })
