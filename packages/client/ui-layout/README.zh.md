...
在 `MOBILE_MAX`（640px）及以下，求解器不再产出分栏布局：会话占据整个 viewport，详情栏保持关闭，侧边栏离开栅格流，变为浮在会话之上、由遮罩承担关闭手势的抽屉。控制栏没有移动端形态，因此关闭的抽屉解析为零宽度，并由 frame 自行渲染打开控件——hero 状态没有会话头部，frame 是回到会话列表的唯一入口。`Columns.overlay` 标记该布局，此时 `Columns.sidebar` 报告的是抽屉宽度而非栏宽度；抽屉受 `DRAWER_MAX` 限制，并始终为会话留出至少 `DRAWER_PEEK` 的可点按区域。跨越断点只改变 frame 几何：slot 占用方保持其树位置与 React 标识，且不会重写任何宽度偏好，因此变宽后会恢复此前的分栏布局。`ctx.layout.collapseNarrowSidebar()` 是导航侧的配套方法——ui-sidebar 与 ui-workspace 在新建或打开会话后调用它，使抽屉不再遮挡目标会话。
...
- **详情栏没有移动端形态**：在 `MOBILE_MAX` 以下它始终关闭，因此工具详情等 `details` 占用方在手机上无法访问；补齐该缺口需要它们自己的移动端表面（覆盖在会话之上的 sheet）。
- **抽屉没有滑动手势**：它只能通过 frame 的打开控件、遮罩和导航开合；边缘滑动需要一个 frame 目前不具备的手势归属方。
