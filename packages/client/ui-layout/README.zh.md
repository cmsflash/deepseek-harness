...
在不超过 `MOBILE_MAX`（640px）时，求解器不再产生栏式布局：会话占据整个视口，右栏保持关闭，侧边栏脱离网格流，成为一个浮在会话之上、背后带关闭遮罩的抽屉。控制栏没有移动端形态，因此关闭的抽屉解析为零宽度，由框架自行渲染打开控件——hero 状态下不存在会话头部，所以框架掌握着回到会话列表的唯一途径。`Columns.overlay` 标记该布局，此时 `Columns.sidebar` 报告的是抽屉宽度而非轨道宽度；抽屉受 `DRAWER_MAX` 限宽，并始终留出至少 `DRAWER_PEEK` 的会话可触及。跨越断点只改变框架几何：槽位占用者保持树位置与 React 身份，任何宽度偏好都不会被改写，因此加宽后恢复先前的栏式布局。每次 `selectPanel` 调用——包括每次会话导航最终到达的 `null`——都会关闭已打开的抽屉，使其不再遮挡目标；在两个断点之间，侧边栏是用户主动打开的一栏，导航不会碰它。
...
- **极窄窗口**——右侧面板关闭后，中栏仍可能小于 400px；左侧 56px 控制栏一直保留到手机断点。
- **右栏没有移动端形态**——低于 `MOBILE_MAX` 时它始终关闭，因此文件预览与其他 `rightbar` 占用者在手机上不可达；在这一缺口补上之前，它们需要自己的移动端界面（覆盖在会话之上的 sheet）。
- **抽屉没有滑动手势**——它通过框架的打开控件、遮罩与导航打开和关闭；边缘滑动需要框架目前没有的手势归属方。
