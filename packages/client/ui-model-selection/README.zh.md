...
模型按提供方分组。composer 菜单只显示模型与推理强度名称。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。弹窗应用为所选模型记住的推理强度，否则应用其默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。

选择一个推理强度会把它按精确的 `provider/model` 路由记录到持久化的 `ui-model-selection` 设置段的 `rememberedEfforts` 中，因此切换到某个模型时会预选上次在它上面运行的等级，而不是适配器默认值；两个入口读写同一份记忆，且它比会话、浏览器和 Host 进程都活得更久。这份记忆是建议性的：适配器不再公布的已记住等级回退到声明的默认值，只有 Host 接受的等级才会被记录，被拒绝的设置写入永不使已经落地的模型切换失败，`null` 记录用户明确选择提供方默认值——这与缺失的键不同。node 半边注册该设置段；没有设置提供方时，适配器默认值照旧生效且不记录任何内容。
...
- **已记住的推理强度不可由用户编辑**——该设置段没有设置行，因此更正它意味着再次选择该等级或编辑设置文档；没有设置提供方的部署保留适配器默认值，且静默地不记录任何内容。
