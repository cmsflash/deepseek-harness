...
Each `ConversationMatch` retains the original accepted Client entry as `record`; its `event` is the same object as `record.event`. Definitions can consume Host history metadata without changing durable events or rebuilding it from incomplete history. Append, prepend, location refresh, and replay preserve that record identity; a replacement window replaces its records.
