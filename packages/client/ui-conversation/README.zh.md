...
每个 `ConversationMatch` 用 `record` 保留原始的、已接受的 Client entry；其 `event` 与 `record.event` 是同一个对象。Definition 可以消费 Host 历史元数据，无需改变持久事件或从不完整的历史重新计算。Append、prepend、location 刷新及重放保留该记录的 identity；替换窗口则替换其记录。
