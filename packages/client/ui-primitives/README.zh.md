...
Markdown 表格按列数决定尺寸（与 deepsuite chat 一致）：少于四列或位于 blockquote 内的表格填满所在栏，单元格文字向下换行至最小可读宽度；四列及以上的表格保持自然宽度，在滚动容器内横向滚动，并在其外层块上携带稳定的 `md-table-wide` 类，供宿主布局把它加宽到栏外；宽表格的横向滚动条在悬停或键盘聚焦时才显现（滚动容器带 `tabindex="0"`），而非常驻绘制。每个已落定的表格还携带一个悬停显现的复制控件，把它写回为 GFM——规范化的空格、由解析得到的 `align` 重建的分隔行、以及由语法重新转义的单元格标记——因此一张对比矩阵粘贴进编辑器时仍是表格，而不是被拍平的单元格文本。该控件位于滚动容器之外，宽表格滚动时它固定在角落；回复流式进行中（末行仍在增长）以及序列化器无法处理的单元格出现时，控件会被扣留（[决策记录](../../../.agents/notes/implemented/feature/2026-08-24-markdown-table-copy-as-markdown.zh.md)）。
