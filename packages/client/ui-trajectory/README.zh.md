...
记录表需要每个步骤的全部事件。订阅 Trajectory 数据源会在其余生命周期内向 Session 要求完整历史，因此以折叠步骤详情打开的窗口会先补回被省略的事件，再由视图呈现。只要仍有轮次省略了步骤，时间线与记录表就隐藏在加载状态之后；补回失败时显示失败信息与错误码，并提供重复该要求的“重试”控件。区间完整后，常规的尾部跟随与加载更早分页保持不变。
...
回复尚未加载的请求头显示为仅含请求头的助手请求行；只有记录用途为压缩的请求才带有压缩标签。完整的追加提示词在请求头未加载时显示为独立系统行；仅提供已知文本，不推断请求选项或工具目录。补入其请求历史后，该独立展示被替代而不重复提示词。历史中的系统提示词变更与最近的请求状态比较，包括没有新请求头的先前提示词更新。每个请求保留其所在位置生效的提示词与变更。包括压缩在内的 surface 替换会恢复最后一个非空的存活系统提示词，即使没有新的系统事件；未加载的提示词在对应分页到达前仍不可用。

完整历史解析原先未加载的前驱后，临时系统请求头可能不再需要。它已物化的键会保留为隐藏状态，而不是被撤回；隐藏贡献既不影响请求解释，也不进入可见记录表。
