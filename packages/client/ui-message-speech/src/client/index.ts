 ...
 * whatever was playing. Audio is fetched through the generated speechCache
 * Remote; the Host resolves the spoken text from its own Session log, so this
 * half sends message identity and never prose.
 * @module @deepseek-ai/dsh-client-ui-message-speech/client
 */
...
export const inject = ['slots', 'remote', 'remote.speechCache', 'locale']
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'ui-message-speech: dictionaries')
  ...
  const playerFor = (sessionId: SessionId): SpeechPlayer => {
    ...
    if (player === undefined) {
      player = new SpeechPlayer(async (messageId) => {
        const reply = await ctx.remote.speechCache.audio({ sessionId, messageId })
