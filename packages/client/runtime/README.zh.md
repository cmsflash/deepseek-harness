...
`Session.setStepDetail` 决定每一页携带多少 step 内容，`SessionRuntime` 为所有会话保存该选择，因此稍后打开的会话与已经打开的会话按相同方式分页。折叠的分页携带 step 的边界并扣下其内部内容，这让窗口在 Definition 读取的 Turn 与 Step 坐标上保持连续；`Session.expandTurn` 回读单个 turn 被扣下的事件，`spliceInterior` 按 seq 合并它们，且不移动窗口的任何一端。`stepDigests` 报告仍被扣下的 step，`stepAccounts` 报告已收到的每个逐 step 账目，因此摘要行的指标在展开后保持不变。
