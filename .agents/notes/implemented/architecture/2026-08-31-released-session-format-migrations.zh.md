...
v0-to-v1 除了有限的 released-v0 归一化外，会保留逻辑 header、seq、引用、时间戳与 payload。它转换已移除的 `steering/message` 与 `compact/*` 事件名称，接受出现在对应 `step/end` 之后的已发布 `llm/retry`，按 turn／step／provider／policy chain 为缺失的 `llm/retry.retryId` 确定性补值，为省略 id 的旧 compaction group 确定性补充同一个 `compactionId`，在键集合不变的前提下把已发布的 `subagent/descriptor` version 2 标记为 version 3，并接受 token usage 上已发布的 `costUsd` 计费金额。已发布的 v0 writer（descriptor version 2 来自 `dsh-v0.1.1-rc.2` 及更早版本；计费金额来自所有携带它的 v0 writer）都产生过这两种变体，拒绝它们只会让已发布的数据无法读取，而不是保护它。v1-to-v2 负责 attempt folding 与引用重写，并且只 emit 已结算的 v2 event。它会把旧的 goal 来源 user message 拆成 `goal/change` 与原本的模型可见 message。它还会为一种有限的已发布 restart 插入 interrupted `turn/end`：一个没有 open step 的 open turn 后出现非空 `next-turn` inbox splice，随后直接开始编号连续的下一轮。
