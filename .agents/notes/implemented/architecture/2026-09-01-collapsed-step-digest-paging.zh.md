...
`session.page` 与 `session.follow` 接受 `stepDetail: 'full' | 'collapsed'`。在 `collapsed` 下，Host 把一个可省略的 step 只作为它的 `step/start` / `step/end` 边界外加一个 `StepDigest` 下发，扣住内部内容，直到 `session.expandSteps` 来取。这些页把 `PAGE_MESSAGES`（50）换成 `COLLAPSED_PAGE_MESSAGES`（300），因为目的是回溯得更远，而不是发送更小的响应。

**边界永不省略，且每条下发记录覆盖其前面被扣住的事件。** Gateway 的 journal stream 只在页内记录的闭区间首尾相接时才接纳该页，因此折叠页通过 `SessionHistoryRecord.covers` 声明它所代表的范围（紧邻记录之前被扣住的事件，以及页末记录之后的那些），而不是把它们发出去：journal 的连续性与缺口修复检查原样成立，`baseSeq` 取覆盖范围的头部而非首个事件，窗口在 Conversation Definition 读取的坐标系中不留空洞。展开发布一次 `splice` 窗口变更，assembler 把它送入既有的按 seq 合并的 `prepend` 路径——展开所能到达的每个 Context 都已持有自己的 start Match，因为一个 Step 的 `step/start` 随该页一同到达。折叠后会变空的页整页下发。
