...
- 切换呈现模式时 Sidebar 面板一动不动，两种模式的平移一模一样；切换时只有会话区在动。隐藏的面板保持 tab 挂载，预览在折叠后仍在；frame 会把这块已挂载的面板裁出自身溢出范围（[折叠的右侧面板不计入 frame 溢出](../bug-fix/2026-09-11-collapsed-rightbar-frame-overflow.zh.md)）。
