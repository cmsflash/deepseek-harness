...
- The Sidebar panel never moves when the presentation switches, and its slide is the same in both presentations; the conversation is the only thing that animates on a switch. A hidden panel keeps its tabs mounted, so a preview survives a collapse; the frame clips that mounted panel out of its overflow ([collapsed right panel outside the frame's overflow](../bug-fix/2026-09-11-collapsed-rightbar-frame-overflow.md)).
