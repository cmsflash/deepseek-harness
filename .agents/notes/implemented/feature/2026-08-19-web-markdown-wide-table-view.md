...
**The parity fixtures change deliberately.** The table-containing markdown-dom fixtures pin the discriminator: `tableScroll tableFill` for narrow and blockquote tables, `tableScroll tableScrollWide` inside a `tableBlock md-table-wide` block for wide ones, and a new `table-wide-and-blockquote` corpus document pins both arms of the blockquote exemption. The block wrapper, and the hook's place on it rather than on the scroller, arrive with the [copy-as-markdown control](2026-08-24-markdown-table-copy-as-markdown.md).
