...
| `ctx.tts` | `seam` | [`speech`](../packages/tts/tts) | [`openai-tts`](../packages/tts/openai-tts) | [`read-aloud`](../packages/tts/read-aloud) | - | 提供方注册到同一个 ctx.speech seam；seam 在任何提供方运行前先把部署策略解析成 spec。 |
| `ctx.readAloud` | `core` | [`read-aloud`](../packages/tts/read-aloud) | - | [`client-read-aloud`](../packages/client/read-aloud) | - | 合成每个已完成轮次并把缓存音频提供给浏览器；音频绝不进入会话日志。 |
